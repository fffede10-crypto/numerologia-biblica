...
export default async function SituacionPage() {
  ...
  const { data: usuario } = await supabaseAdmin
    .from('usuarios')
    .select('area_orientacion, etapa_vida')
    .eq('id', payload.sub)
    .single()
    ...
    <div className="max-w-xl mx-auto px-4 py-10">
      <div className="text-center mb-8">
        ...
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">
          {isActualizando ? 'Actualizar mi contexto' : 'Tu situación de vida'}
        </h1>
      ...
      <SituacionForm
        areaInicial={usuario?.area_orientacion ?? null}
        etapaInicial={usuario?.etapa_vida ?? null}
